...
with quadrante:
    ...
    ax.set_ylabel('Área')
    st.title('Quadrante de Área e População de Estados')
    st.write('Este gráfico mostra a relação entre a área e a população de cada estado brasileiro.')
    for i, txt in enumerate(estados['sigla']):
        ax.text(x[i], y[i], txt, ha='center', va='center', color='black')
